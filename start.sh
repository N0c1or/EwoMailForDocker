...
docker cp ./CentOS-Base.repo ewomail:/etc/yum.repos.d/CentOS-Base.repo
docker cp ./epel.repo ewomail:/etc/yum.repos.d/epel.repo
docker exec -d --privileged ewomail /bin/sh -c 'yum install dbus && mkdir -p /run/dbus && dbus-daemon --system --nofork &';
